...
const NavButton = ({ x, y, label, icon, newTab, link ,labelDirection="right"}) => {
  return (
   <ResponsiveComponent>
   {({size}) =>{
    ...
    >
      <Link
        ...
        name={label}
      >
        <span className="relative  w-12 h-12 p-3 animate-spin-slow-reverse group-hover:pause hover:text-accent">
          {getIcon(icon)}
          {/* Tooltip */}
    ...
    >
      <Link
        ...
        name={label}
      >
        <span className="relative w-10 h-10  xs:w-12 xs:h-12 p-2.5 xs:p-3  hover:text-accent">
          {getIcon(icon)}
          {/* Tooltip */}
          <span className="peer bg-transparent absolute top-0 left-0 w-full  h-full"/>
...
// import { Github, Home, Link, Linkedin, NotebookText, Palette, Phone, Twitter, User } from 'lucide-react';
// import React from 'react'
